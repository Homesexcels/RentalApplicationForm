...
export default defineConfig(({ mode }) => {
  ...
  return {
    // FIX applied here: Uses your repository name as the base URL context when built on GitHub
    base: process.env.GITHUB_REPOSITORY ? `/${process.env.GITHUB_REPOSITORY.split('/')[1]}/` : (process.env.FIGMA_PUBLIC_URL ? `${process.env.FIGMA_PUBLIC_URL}/` : '/'),
    build: {
...
/** Error overlay caching functions */
...
// Fallback functions stubbed below to keep file structurally sound and complete
function figmaReactRefreshBoundaryFallback(): Plugin {
  return { name: 'figma-react-refresh-boundary-fallback' }
}
function figmaMakeKitPlugin(options: { storiesGlob: string }): Plugin {
  return { name: 'figma-make-kit-plugin' }
}
